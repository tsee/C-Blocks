package C::Blocks::CSignatureParser;
use strict;
use warnings;
...
sub extract_function_metadata {
	...
	while( $code =~ m{ $re_plausible_place_to_begin_a_declaration
...
C::Blocks::CSignatureParser - (Partial) C function signature parsing with regexes

=head1 SYNOPSIS

  my $result = C::Blocks::CSignatureParser::extract_function_metadata($code);

  # This will only parse the first declaration that's found
  my $result = C::Blocks::CSignatureParser::extract_function_metadata($code, {first_only => 1});
